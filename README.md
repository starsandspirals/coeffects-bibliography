# Coeffects Bibliography

Inspired by the collaborative [effects bibliography](https://github.com/yallop/effects-bibliography),
this is a collection of research papers and other resources related to coeffect tracking
in programming languages theory and implementation.

### Contributing guidelines

* If you don't have push access, you can [create a pull request](https://github.com/plclub/coeffects-bibliography/pulls).
* For publications, add an entry to this file **and** the `coeffects.bib` file,
  both sorted by date in newest → oldest order,
  then for ambiguous dates by article-insensitive alphabetical order
  (e.g. _A Core Quantitative..._ comes after _Bounded Linear..._).
* Use consistent formatting with the existing files.

## Software Projects

* **Granule**: A statically-typed linear functional language with graded modal types for fine-grained program reasoning. \
  [[website](https://granule-project.github.io/index.html)] [[GitHub](https://github.com/granule-project/granule)]

## Papers and Preprints

### 2026

* **On Graded Coeffect Types for Information-Flow Control** \
  Vilem-Benjamin Liepelt, Danielle Marshall, Dominic Orchard, Vineet Rajani, and Michael Vollmer \
  [[DOI](https://doi.org/10.1007/978-3-032-08187-2_7)]

### 2024

* **Non-linear communication via graded modal session types** \
  Danielle Marshall and Dominic Orchard \
  [[DOI](https://doi.org/10.1016/j.ic.2024.105234)]

* **Effects and Coeffects in Call-by-Push-Value** (OOPSLA 2024) \
  Cassia Torczon, Emmanuel Suárez Acevedo, Shubh Agrawal, Joey Velez-Ginorio, and Stephanie Weirich \
  [[DOI](https://doi.org/10.1145/3689750)]
  [[arXiv (extended version)](https://arxiv.org/abs/2311.11795)]

* **Functional Ownership through Fractional Uniqueness** (OOPSLA 2024) \
  Danielle Marshall and Dominic Orchard \
  [[DOI](https://doi.org/10.1145/3649848)]
  [[appendices](https://zenodo.org/records/10799026)]
  [[artefact](https://zenodo.org/records/10797791)]

* **Program Synthesis from Graded Types** (ESOP 2024) \
  Jack Hughes and Dominic Orchard \
  [[DOI](https://doi.org/10.1007/978-3-031-57262-3_4)]
  [[appendices](https://zenodo.org/records/10594378)]

### 2023

* **Resource-Aware Soundness for Big-Step Semantics** (OOPSLA 2023) \
  Riccardo Bianchini, Francesco Dagnino, Paola Giannini, and Elena Zucca \
  [[DOI](https://doi.org/10.1145/3622843)]
  [[arXiv (extended version)](https://arxiv.org/abs/2507.13792v4)]

* **A Graded Modal Dependent Type Theory with a Universe and Erasure, Formalized** (ICFP 2023) \
  Andreas Abel, Nils Anders Danielsson, and Oskar Eriksson \
  [[DOI](https://doi.org/10.1145/3607862)]
  [[artefact](https://graded-type-theory.github.io/graded-type-theory/icfp23/)]
  [[GitHub](https://github.com/graded-type-theory/graded-type-theory)]

* **Combining Dependency, Grades, and Adjoint Logic** (TyDe 2023) \
  Peter Hanukaev and Harley Eades III \
  [[DOI](https://doi.org/10.1145/3609027.3609408)]

* **Multi-Graded Featherweight Java** (ECOOP 2023) \
  Riccardo Bianchini, Francesco Dagnino, Paola Giannini, and Elena Zucca \
  [[DOI](https://doi.org/10.4230/LIPIcs.ECOOP.2023.3)]
  [[arXiv (extended version)](https://arxiv.org/abs/2302.07782)]

### 2022

* **Linearity and Uniqueness: An Entente Cordiale** (ESOP 2022) \
  Danielle Marshall, Michael Vollmer, and Dominic Orchard \
  [[DOI](https://doi.org/10.1007/978-3-030-99336-8_13)]
  [[appendices](https://zenodo.org/records/5919194#.ZB3ZlOzP1jw)]

* **A Relational Theory of Effects and Coeffects** (POPL 2022) \
  Ugo Dal Lago and Francesco Gavazzo \
  [[DOI](https://doi.org/10.1145/3498692)]
  [[HAL](https://inria.hal.science/hal-03923470)]

### 2021

* **Linear Exponentials as Graded Modal Types** (TLLA 2021) \
  Jack Hughes, Danielle Marshall, James Wood, and Dominic Orchard \
  [[HAL](https://hal-lirmm.ccsd.cnrs.fr/lirmm-03271465v1)]

* **Graded Modal Dependent Type Theory** (ESOP 2021) \
  Benjamin Moon, Harley Eades III, and Dominic Orchard \
  [[DOI](https://doi.org/10.1007/978-3-030-72019-3_17)]
  [[arXiv (extended version)](https://arxiv.org/abs/2010.13163)]

* **A Graded Dependent Type System with a Usage-Aware Semantics** (POPL 2021) \
  Pritam Choudhury, Harley Eades III, Richard A. Eisenberg, and Stephanie Weirich \
  [[DOI](https://doi.org/10.1145/3434331)]
  [[arXiv (extended version)](https://arxiv.org/abs/2011.04070)]

### 2020

* **Resourceful Program Synthesis from Graded Linear Types** (LOPSTR 2020) \
  Jack Hughes and Dominic Orchard \
  [[DOI](https://doi.org/10.1007/978-3-030-68446-4_8)]

* **Deriving Distributive Laws for Graded Linear Types** (TLLA 2020) \
  Jack Hughes, Michael Vollmer, and Dominic Orchard \
  [[arXiv](https://arxiv.org/abs/2112.14966)]
  [[extended abstract](http://granule-project.github.io/papers/deriving-graded-dist.pdf)]

* **A Unified View of Modalities in Type Systems** (ICFP 2020) \
  Andreas Abel and Jean-Philippe Bernardy \
  [[DOI](https://doi.org/10.1145/3408972)]
  [[PDF (extended version)](https://www.cse.chalmers.se/~abela/icfp20-long.pdf)]

### 2019

* **Quantitative Program Reasoning with Graded Modal Types** (ICFP 2019) \
  Dominic Orchard, Vilem-Benjamin Liepelt, and Harley Eades III \
  [[DOI](https://doi.org/10.1145/3341714)]
  [[PDF (extended version)](https://kar.kent.ac.uk/74450/1/paper.pdf)]

### 2018

* **Syntax and Semantics of Quantitative Type Theory** (LICS 2018) \
  Robert Atkey
  [[DOI](https://doi.org/10.1145/3209108.3209189)]
  [[PDF](https://bentnib.org/quantitative-type-theory.pdf)]

* **Linear Haskell: Practical Linearity in a Higher-Order Polymorphic Language** (POPL 2019) \
  Jean-Philippe Bernardy, Mathieu Boespflug, Ryan R. Newton, Simon Peyton Jones, and Arnaud Spiwack \
  [[DOI](https://doi.org/10.1145/3158093)]
  [[PDF (extended version)](https://www.microsoft.com/en-us/research/wp-content/uploads/2017/03/hlt.pdf)]
  [[GHC wiki](https://gitlab.haskell.org/ghc/ghc/-/wikis/linear-types)]

### 2016

* **Combining Effects and Coeffects via Grading** (ICFP 2016) \
  Marco Gaboardi, Shin-ya Katsumata, Dominic Orchard, Flavien Breuvart, and Tarmo Uustalu \
  [[DOI](https://doi.org/10.1145/2951913.2951939)]
  [[PDF](https://cs-people.bu.edu/gaboardi/publication/GaboardiEtAlIicfp16.pdf)]

* **I Got Plenty o' Nuttin'** \
  Conor McBride \
  [[DOI](10.1007/978-3-319-30936-1_12)]
  [[PDF](https://personal.cis.strath.ac.uk/conor.mcbride/PlentyO-CR.pdf)]

### 2014

* **Bounded Linear Types in a Resource Semiring** (ESOP 2014) \
  Dan R. Ghica and Alex I. Smith \
  [[DOI](https://doi.org/10.1007/978-3-642-54833-8_18)]
  [[PDF](https://research.birmingham.ac.uk/files/23697858/esop14.pdf)]

* **A Core Quantitative Coeffect Calculus** (ESOP 2014) \
  Aloïs Brunel, Marco Gaboardi, Damiano Mazza & Steve Zdancewic \
  [[DOI](10.1007/978-3-642-54833-8_19)]
  [[PDF](https://www.lipn.fr/~mazza/papers/CoreQuantCoeff.pdf)]

* **Coeffects: a calculus of context-dependent computation** (ICFP 2014) \
  Tomáš Petříček, Dominic Orchard, and Alan Mycroft \
  [[DOI](https://doi.org/10.1145/2628136.2628160)]

### 2013

* **Coeffects: Unified Static Analysis of Context-Dependence** (ICALP 2013) \
  Tomáš Petříček, Dominic Orchard, and Alan Mycroft \
  [[DOI](https://doi.org/10.1007/978-3-642-39212-2_35)]

## Theses and Dissertations

### 2025

* **Graded Modal Type Theory, Formalized** (licentiate thesis) \
  Oskar Eriksson \
  [[HDL](https://hdl.handle.net/2077/86472)]
  [[PDF](https://gupea.ub.gu.se/server/api/core/bitstreams/14d8fa35-2c02-4466-b61b-83cfade759b5/content)]
  [[artefact](https://graded-type-theory.github.io/graded-type-theory/lic2025/)]
